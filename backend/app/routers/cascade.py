...
from app.schemas.cascade import (
    CardData, CascadeResponse, FinancialSummary,
    PointsInput, SimulateRequest,
)
from app.services.calculator import (
    HUDO_NAMES, LAYER3_META, MID_META, REVENUE_M,
    ROE_CURRENT, ROE_TARGET_2027, ROIC_CURRENT, ROIC_TARGET_2027,
    SALES_TARGET_OKU,
    CascadeResult, KpiResult, calculate,
)
...
def _kpi_to_card(node: KpiResult, indicator: Indicator | None,
                 column_key_default: str) -> CardData:
    return CardData(
...
def _financial_to_cards(result: CascadeResult,
                        indicator_map: dict[str, Indicator]) -> list[CardData]:
    cards: list[CardData] = []
    ...
    for calc_id, default_label, desc in drivers_meta:
        ...
        cards.append(CardData(
            ...
            projected=delta, improvement=delta,
            description=desc,
            unit="%",
        ))
...
def _load_indicators() -> dict[str, Indicator]:
    ...
    with eng.connect() as conn:
        rows = conn.execute(select(Indicator).where(Indicator.calc_id.is_not(None))).all()
        for row in rows:
            ind = row[0] if isinstance(row, tuple) else row
            if ind.calc_id:
                out[ind.calc_id] = ind
    return out

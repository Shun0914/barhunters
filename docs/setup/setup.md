...
- **ログイン**: フロント `http://localhost:3000/login` — ログイン ID は `users.employee_code`（例: `MIN-KC-001`）、パスワードは `backend/.env` の `DEMO_PASSWORD`（未設定時 `demo`）。
- **セッション**: API が **HttpOnly Cookie** に JWT を載せる。有効期限は `JWT_EXPIRE_MINUTES`（既定 480 分）。
- **ログアウト**: サイドバー左下の「ログアウト」で Cookie を削除し `/login` へ。
- **役割の切替（デモ）**: `ALLOW_DEV_AUTH_HEADER=true`（API）かつ `NEXT_PUBLIC_ALLOW_DEV_USER_SWITCH=true`（FE）のとき、ログイン後に左下ドロップダウンで別ユーザーとして表示切替（`X-Dev-User-Id`）。本番 SSO はスコープ外。
- **Azure 等で FE/API が別ドメイン**のときは `SESSION_COOKIE_SAMESITE=none` と `SESSION_COOKIE_SECURE=true` を API 側に設定し、`ALLOW_ORIGINS` にフロントの URL を含める。
